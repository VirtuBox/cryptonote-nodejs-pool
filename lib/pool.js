...
if (cnAlgorithm == "cryptonight" || cnAlgorithm == "cryptonight_light") {
    cryptoNight = cnHashing[cnAlgorithm];
} else if (cnAlgorithm == "cryptonight_heavy") {
    cryptoNight = multiHashing['cryptonight_heavy'];
}
if (!cryptoNight) {
            ...
            redisClient.hgetall(config.coin + ':notifications', function(error, data) {
                ...
                for (var address in data) {
                    ...
                    emailSystem.sendEmail(
                        ...
                        'Block found!',
                        'block_found',
                        { 'BLOCK_HEIGHT': job.height, 'BLOCK_TIME': dateFormat(Date.now(), 'yyyy-mm-dd HH:MM:ss Z') }
                    );
                }
            });
        }
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if (cnAlgorithm != "cryptonight" || config.cnVariant && config.cnVariant < 0) {
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var workerName = '';
            if (params.rigid) {
                workerName = cleanupSpecialChars(params.rigid.trim());
            }
            else if (pass) {
		...
		workerName = cleanupSpecialChars(workerName);
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        ...
        if (portData.ssl) {
            if (!config.poolServer.sslCert) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): No SSL certificate specified', [portData.port]);
                cback(true);
            } else if (!config.poolServer.sslKey) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): No SSL key specified', [portData.port]);
                cback(true);
            } else if (!config.poolServer.sslCA) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): No SSL certificate authority specified', [portData.port]);
                cback(true);
            } else if (!fs.existsSync(config.poolServer.sslCert)) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): SSL certificate file not found', [portData.port]);
                cback(true);
            } else if (!fs.existsSync(config.poolServer.sslKey)) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): SSL key file not found', [portData.port]);
                cback(true);
            } else if (!fs.existsSync(config.poolServer.sslCA)) {
                log('error', logSystem, 'Could not start server listening on port %d (SSL): SSL certificate authority file not found', [portData.port]);
                cback(true);
            } else {
                var options = {
