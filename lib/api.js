...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    var minLevel = config.payments.minPayment / config.payments.denomination;
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        ...
        var payoutLevel = level * config.payments.denomination;
...
function handleGetMinerPayoutLevel(urlParts, response){
    ...
    redisClient.hget(config.coin + ':workers:' + address, 'minPayoutLevel', function(error, value){
        ...
        var currentLevel = value / config.payments.denomination;
...
if (config.api.ssl){
    if (!config.api.sslCert) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): No SSL certificate specified', [config.api.sslPort]);
    } else if (!config.api.sslKey) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): No SSL key specified', [config.api.sslPort]);
    } else if (!config.api.sslCA) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): No SSL certificate authority specified', [config.api.sslPort]);
    } else if (!fs.existsSync(config.api.sslCert)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate file not found', [config.api.sslPort]);
    } else if (!fs.existsSync(config.api.sslKey)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL key file not found', [config.api.sslPort]);
    } else if (!fs.existsSync(config.api.sslCA)) {
        log('error', logSystem, 'Could not start API listening on port %d (SSL): SSL certificate authority file not found', [config.api.sslPort]);
    } else {
        var options = {
